...
def _parse_codes(codes: Optional[str]) -> List[str]:
    if not codes:
        return []
...
def _resolve_codes(cfg, codes: Optional[str], as_of: date, limit: int) -> List[str]:
...
@app.command()
def fetch(
    codes: Optional[str] = typer.Option(None, "--codes", help="逗号分隔股票代码；缺省=全市场"),
...
def backtest(
    ...
    strategy_path: Optional[str] = typer.Option(None, "--strategy-path", help="私有策略外部目录；缺省用内置示例"),
    start: str = typer.Option(..., "--start", help="开始日期 YYYY-MM-DD"),
    end: str = typer.Option(..., "--end", help="结束日期 YYYY-MM-DD"),
    codes: Optional[str] = typer.Option(None, "--codes", help="逗号分隔股票代码；缺省用 Universe"),
    ...
    position_size: Optional[float] = typer.Option(None, "--position-size", help="单票目标权重；缺省取 risk.max_single_position"),
    output: Optional[str] = typer.Option(None, "--output", help="将净值曲线写出到该 CSV"),
    config: Optional[str] = typer.Option(None, "--config", help="配置文件路径"),
):
    """运行回测并打印绩效摘要。"""
...
def optimize(
    ...
    strategy_path: Optional[str] = typer.Option(None, "--strategy-path", help="私有策略外部目录"),
